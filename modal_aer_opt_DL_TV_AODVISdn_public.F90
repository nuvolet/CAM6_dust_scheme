...
! This version is combination from one made from Danny Leung (! dmleung 17 Nov 2023) for the Dust Scheme 
! and Toni viudez for aerosols nudging scheme where making binterp, modal_size_parameters, read_water_refindex subroutines as public
...
implicit none
public :: modal_aer_opt_readnl, modal_aer_opt_init, modal_aero_sw, modal_aero_lw, binterp, modal_size_parameters, read_water_refindex, get_aodvisdn

real(r8), allocatable, save :: saved_aodvis(:,:)  ! (pcols, begchunk:endchunk)
! public :: get_aodvisdn ! make get_aodvis public so it can be called from other modules.
!private
save
...
! Add AODVISdn_computed as a module-level variable
real(r8), allocatable, public :: AODVISdn_computed(:)  ! Aerosol optical depth 550 nm, day night
...
subroutine modal_aer_opt_init()
   ...
   call addfld ('EXTINCT',    (/ 'lev' /), 'A','/m','Aerosol extinction 550 nm, day only',                   &
                flag_xyfill=.true.)
   call addfld ('EXTINCTDUST',    (/ 'lev' /), 'A','/m','Dust extinction 550 nm, day only',                   &
                flag_xyfill=.true.)  ! dmleung 17 Nov 2023
   ...
   call addfld ('EXTINCTdn',    (/ 'lev' /), 'A','/m','Aerosol extinction 550 nm, day night',                 &
                  flag_xyfill=.true.)
   call addfld ('EXTINCTDUSTdn',   (/ 'lev' /), 'A','/m','Dust extinction 550 nm, day night',                 &
                  flag_xyfill=.true.) ! dmleung 17 Nov 2023
   ...
   call addfld ('ABSORBdn',     (/ 'lev' /), 'A','/m','Aerosol absorption, day night',                        &
                  flag_xyfill=.true.)
   call addfld ('QAERWATdn',    (/ 'lev' /), 'A','kg/kg','Aerosol water mixing ratio, day night',             &
                flag_xyfill=.true.)  ! dmleung 17 Nov 2023
   call addfld ('AODVISdn',     horiz_only,  'A','  ','Aerosol optical depth 550 nm, day night',              &
                  flag_xyfill=.true.)

   call addfld ('AODVISdn_computed',     horiz_only,  'A','  ','Aerosol optical depth 550 nm, computed day night',              &
                  flag_xyfill=.true.)
   ...
   call addfld ('AODABSBC',      horiz_only, 'A','  ',    'Aerosol absorption optical depth 550 nm from BC, day only',&
        flag_xyfill=.true.)
   call addfld ('AODABSDUST',    horiz_only, 'A','  ',    'Aerosol absorption optical depth 550 nm from dust, day only',&
        flag_xyfill=.true.)   ! dmleung 17 Nov 2023
   ...
   call addfld ('AODABSBCdn',      horiz_only, 'A','  ',    'Aerosol absorption optical depth 550 nm from BC, day night',&
        flag_xyfill=.true.)
   call addfld ('AODABSDUSTdn',    horiz_only, 'A','  ',    'Aerosol absorption optical depth 550 nm from dust, day night',&
        flag_xyfill=.true.)   ! dmleung 17 Nov 2023
   ...
   if (history_aero_optics) then 
      call add_default ('AODDUST1'     , 1, ' ')
      call add_default ('AODDUST2'     , 1, ' ')   ! dmleung 17 Nov 2023
      call add_default ('AODDUST3'     , 1, ' ')
      ...
      call add_default ('AODABSDUST'   , 1, ' ')   ! dmleung 17 Nov 2023
      call add_default ('AODDUST'      , 1, ' ')
      ...
      call add_default ('EXTINCTDUST'  , 1, ' ')   ! dmleung 17 Nov 2023
      ...
      call add_default ('EXTINCTDUSTdn'  , 1, ' ')   ! dmleung 17 Nov 2023
   ...
   do ilist = 1, n_diag
      if (call_list(ilist)) then
         ...
         call addfld ('EXTINCTDUST'//diag(ilist),(/ 'lev' /), 'A','/m', & 
              'Dust extinction 550 nm, day only', flag_xyfill=.true.)  ! dmleung 17 Nov 2023
         ...
         call addfld ('EXTINCTDUSTdn'//diag(ilist),(/ 'lev' /), 'A','/m', & 
              'Dust extinction 550 nm, day night', flag_xyfill=.true.)  ! dmleung 17 Nov 2023
         call addfld ('ABSORBdn'//diag(ilist),     (/ 'lev' /), 'A','/m',&
              'Aerosol absorption, day night',  flag_xyfill=.true.)
         call addfld ('QAERWATdn'//diag(ilist),(/ 'lev' /), 'A','kg/kg', &
              'Aerosol water mass mixing ratio, day night', flag_xyfill=.true.)  ! dmleung 17 Nov 2023
         call addfld ('AODVISdn'//diag(ilist),     horiz_only,  'A','  ',&
              'Aerosol optical depth 550 nm, day night',  flag_xyfill=.true.)

         call addfld ('AODVISdn_computed'//diag(ilist),     horiz_only,  'A','  ',&
              'Aerosol optical depth 550 nm, computed day night',  flag_xyfill=.true.)
         ...
         if (history_aero_optics) then
            ...
            call add_default ('EXTINCTDUST'//diag(ilist), 1, ' ')    ! dmleung 17 Nov 2023
            call add_default ('ABSORB'//diag(ilist),  1, ' ')
            !call add_default ('QAERWATdn'//diag(ilist),  1, ' ')       ! dmleung 17 Nov 2023
   ...
   ! Add allocation of AODVISdn after other initializations
   ! if (.not. allocated(AODVISdn)) then
   !    allocate(AODVISdn(pcols))
      AODVISdn_computed(:) = 0._r8
   ! end if   
...
subroutine modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, &
   ...
   real(r8) :: extinctdust(pcols,pver)     ! dust extinction, dmleung 17 Nov 2023
   real(r8) :: qaerwat_tot(pcols,pver)     ! aerosol water mass mixing ratio, dmleung 17 Nov 2023
   ...
   real(r8) :: aodabsdust(pcols)           ! absorption optical depth of dust, add by dmleung 17 Nov 2023
   ...
   extinctdust(1:ncol,:) = 0.0_r8   ! dmleung 17 Nov 2023
   qaerwat_tot(1:ncol,:) = 0.0_r8   ! dmleung 17 Nov 2023
   absorb(1:ncol,:)      = 0.0_r8
   aodvis(1:ncol)        = 0.0_r8
   ! AODVISdn(:ncol) = 0.0_r8
   ...
   aodabsdust(:ncol)     = 0.0_r8 ! dmleung 17 Nov 2023
   dustaod(:ncol)        = 0.0_r8
   so4aod(:ncol)         = 0.0_r8
   pomaod(:ncol)         = 0.0_r8
   soaaod(:ncol)         = 0.0_r8
         ...
         do k = top_lev, pver
            ...
            if (savaervis) then
               ...
               do i = 1, ncol
                  ...
                  ! AODVISdn(i)  = AODVISdn(i) + dopaer(i)  ! Changed from aodvis
                  AODVISdn_computed(i)  = AODVISdn_computed(i) + dopaer(i)  ! Changed from aodvis                   ! 
                  ! if (allocated(saved_aodvis)) then
                     saved_aodvis(1:ncol, lchnk) = AODVISdn_computed(1:ncol)
                  ! endif
                  ...
                  if (wetvol(i) > 1.e-40_r8) then
                     ...
                     aodabsdust(i)  = aodabsdust(i) + absdust(i)*dopaer(i)*(1.0_r8-palb(i)) ! dmleung 17 Nov 2023
                     ...
                     if (m == 1 .or. m == 2) then
                       ...
                       extinctdust(i,k) = extinctdust(i,k) + aodc*1.16_r8*air_density(i,k)/mass(i,k) ! dmleung 17 Nov 2023
                       !extinctdust(i,k) = extinct(i,k) * ()
                     else
                       dustaod(i)     = dustaod(i) + (aodc * 1.28_r8)
                       extinctdust(i,k) = extinctdust(i,k) + aodc*1.28_r8*air_density(i,k)/mass(i,k) ! dmleung 17 Nov 2023
                     end if
                     ! --Longlei Li
                     !dustaod(i)     = dustaod(i) + aodc
                     !extinctdust(i,k) = extinctdust(i,k) + dustaod(i)*air_density(i,k)/mass(i,k) ! dmleung 17 Nov 2023
...
!                  if (nerr_dopaer >= nerrmax_dopaer) then
   ...
   ! add a line of output for aerosol water mass mixing ratio   dmleung 20 Nov 2023
   do m = 1, nmodes
      qaerwat_tot(:,:) = qaerwat_tot(:,:) + qaerwat_m(:,:,m)
   end do
   !###############################################################################
   ...
   call outfld('EXTINCTDUSTdn'//diag(list_idx),  extinctdust, pcols, lchnk)   ! dmleung 17 Nov 2023
   ...
   if (list_idx == 0) then
      ...
      call outfld('AODABSDUSTdn',    aodabsdust,    pcols, lchnk)  ! dmleung, 17 Nov 2023
      ...
      do i = 1, nnite
         ...
         aodabsdust(idxnite(i)) = fillvalue   ! dmleung 17 Nov 2023
      ...
      call outfld('AODABSDUST',    aodabsdust,    pcols, lchnk)  ! dmleung 17 Nov 2023
...
!===============================================================================
! Subroutine to get the adosvisdn field for the diagnostic lists.  
!===============================================================================

subroutine get_aodvisdn(aod_output, ncol, lchnk)
   integer, intent(in) :: ncol, lchnk
   real(r8), intent(out) :: aod_output(ncol)
   
   ! if (.not. allocated(saved_aodvis)) then
   ! call endrun('get_aodvisdn: saved_aodvis not allocated')
   ! endif
   
   aod_output(1:ncol) = saved_aodvis(1:ncol, lchnk)
end subroutine get_aodvisdn
